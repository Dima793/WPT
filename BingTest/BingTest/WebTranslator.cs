...
namespace BingTest
{
    class WebTranslator : ITranslator
    {
        ...
        public WebTranslator(string sourceLanguage, string targetLanguage)
        {
        ...
        {
            var task = TranslateAsync(sourceText);
            task.Wait();
            return task.Result;
        }

        public async Task<string> TranslateAsync(string sourceText)
        {
            if (!_initialized)
            {
                //Get Client Id and Client Secret from https://datamarket.azure.com/developer/applications/
                //Refer obtaining AccessToken (http://msdn.microsoft.com/en-us/library/hh454950.aspx) 
                _admAuth = await AdmAuthentication.CreateAsync("ivanov017", "TJ+N2eiMu6vlExCwWdXba7IkAGwyLUt75ic6si1GoIg=");
                _initialized = true;
            }
            ...
            try
            {
                ...
                return await TranslateMethod(headerValue, sourceText);
            }
        ...
        private async Task<string> TranslateMethod(string authToken, string text)
        {
            string uri = "http://api.microsofttranslator.com/v2/Http.svc/Translate?text=" + HttpUtility.UrlEncode(text) + "&from=" + _sourceLanguage + "&to=" + _targetLanguage;
    ...
    public class AdmAuthentication
    {
        ...
        {
            var request = string.Format("grant_type=client_credentials&client_id={0}&client_secret={1}&scope=http://api.microsofttranslator.com", HttpUtility.UrlEncode(clientId), HttpUtility.UrlEncode(clientSecret));
        ...
        private static async Task<AdmAccessToken> HttpPostAsync(string DatamarketAccessUri, string requestDetails)
        {
            ...
            byte[] bytes = StringToAscii(requestDetails);// Encoding.UTF8.GetBytes(requestDetails);
